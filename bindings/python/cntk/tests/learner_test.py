...
def test_learner_init():
    ...
    nesterov(res.parameters, lr=[0.1, 0.2], m_schedule)

    adagrad(res.parameters, lr=[0.1]*3 +[0.2]*2 +[0.3], need_ave_multiplier=True)

    m_schedule = momentum_schedule(momentum_time_constant,  unit=UnitType.minibatch)
    adam_sgd(res.parameters, lr=[(3,0.1), (2, 0.2), (1, 0.3)], momentum=m_schedule)
